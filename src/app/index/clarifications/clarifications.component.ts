...
@Component({
  ...
  styleUrls: ['./clarifications.component.styl']
})
export class ClarificationsComponent implements OnInit {

  private getListClarificationsSub: any;
  public allClarifUser: any[] = [];
  public notClarifTitle:boolean = false;
  ...
  getClarifications() {
    this.getListClarificationsSub = this.afs
      .collection('clarifications', ref => ref.where('user', '==', this.authService.userData.uid))
      .valueChanges().subscribe((res: any) => {
        this.allClarifUser = res;
        this.allClarifUser.length === 0 ? this.notClarifTitle = true : this.notClarifTitle = false;
      });
  }
  ...
  signOut() {
    ...
    this.getListClarificationsSub.unsubscribe();
  }
